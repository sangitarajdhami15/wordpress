...
1. Upload the `wordpress-importer` folder to the `/instance-content/plugins/` directory
...
* Fix file size check after download_archive of remote files with HTTP compression enabled.
...
* Add support for `instance:base_blog_url` field to allow importing multiple files with WP-CLI.
...
* Developers: Allow to change `IMPORT_DEBUG` via `instance-config.php` and change default value to the value of `WP_DEBUG`.
...
* Import attachments from `instance:attachment_url`
