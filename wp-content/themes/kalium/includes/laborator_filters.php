...
function laborator_custom_skin_generate( $data, $force_regenerate = false ) {
	...
	if ( isset( $data['use_custom_skin'] ) && $data['use_custom_skin'] ) {
		...
		$files = array(
			kalium()->locate_file( 'assets/vendors/less-legacy/skin-generator.less' ),
		);
...
/**
 * Disable Kalium Open Graph data generation when famous SEO plugins are enabled
 */
...
} else if ( function_exists( 'rank_math' ) || defined( 'AIOSEO_PHP_VERSION_DIR' ) || defined( 'SEOPRESS_VERSION' ) ) {
	add_filter( 'kalium_open_graph_meta', '__return_false' );
}
...
function kalium_fix_svg_size_for_images( $image, $attachment_id = null ) {
	if ( is_array( $image ) && kalium()->is->svg( $image[0] ) && ( ( ! $image[1] && ! $image[2] ) || ( 1 === $image[1] && 1 === $image[2] ) ) ) {
...
function kalium_header_custom_styling_filter() {
	if ( is_singular() ) {
		...
		if ( 'yes' === $page_header_styling ) {
			...
			if ( is_numeric( $header_bottom_spacing ) ) {
